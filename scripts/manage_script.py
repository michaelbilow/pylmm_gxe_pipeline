...
def main(project_folder, study_name, max_phenos=10, pipeline_step_start=0, run_local=False):
    ...
    if not exists(separate_pheno_folder):
        ...
        sys.stderr.write("Separated pheno folder exists; overwriting\n")
    ...
    if pipeline_step_start <= 1:
        ...
        if not exists(clean_plink_folder):
            ...
            sys.stderr.write("Clean PLINK folder exists; overwritign\n")
            removedirs(clean_plink_folder)
            makedirs(clean_plink_folder)
    ...
    if pipeline_step_start <= 3:
        if not exists(separate_pheno_folder):
            ...
            removedirs(separate_pheno_folder)
            makedirs(separate_pheno_folder)
    ...
    if pipeline_step_start <= 6:
        blank_kinship.main(kinship_fn=[join(clean_plink_folder, x)
    ...
    if run_local:
        if not exists(pylmm_output_folder):
            ...
            removedirs(pylmm_output_folder)
            makedirs(pylmm_output_folder)
...
if __name__ == "__main__":
    proj_folder = '../eqtl'
    study = 'HAEC'
    main(project_folder=proj_folder, study_name=study, pipeline_step_start=6)
